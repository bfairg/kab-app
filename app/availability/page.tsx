...
export default function AvailabilityPage() {
  ...
  function goSignUp() {
    if (!zoneId) return;
    router.push(`/signup?postcode=${encodeURIComponent(postcode)}&zoneId=${encodeURIComponent(zoneId)}`);
  }

  function goWaitingList() {
    router.push(
      `/waiting-list?postcode=${encodeURIComponent(postcode)}${zoneId ? `&zoneId=${encodeURIComponent(zoneId)}` : ""}`
    );
  }
          ...
          <section className="lg:col-span-5">
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.02)]">
              ...
              <div className="mt-2 rounded-xl border border-white/10 bg-black/20 p-4">
                ...
                <div className="mt-1 text-lg font-semibold tracking-tight">{postcode || "Unknown"}</div>
              </div>
              ...
              <div className="mt-4 text-xs text-white/60 space-y-2">
                ...
                <p>
                  If your zone is full, you can join the waiting list and we’ll notify you when space opens.
                </p>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-[0_0_0_1px_rgba(255,255,255,0.02)]">
              <div className="border-b border-white/10 p-6">
                ...
                <div className="mt-1 text-xs text-white/60">
                  {loading ? "Checking your zone..." : error ? "We hit a problem" : available ? "Space available" : "Zone currently full"}
                </div>
              </div>
